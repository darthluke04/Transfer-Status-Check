...
import type { FolderStatus, Status, ValidationRule } from '@/lib/types';
...
export default function FolderFileFinderPage() {
  ...
  const { settings, getInitialTimeRange } = useSettings();
  ...
  const validateFolders = useCallback((files: File[], year: string, date: string) => {
    const newStatuses = folderConfig.map(config => {
      ...
      const isTimeRelevant = !!getInitialTimeRange(config.id);
      ...
      for (const file of files) {
        ...
        for (const rule of config.rules) {
          ...
          if (!patternRegex.test(fileDir)) continue;

          if (rule.fileNameKeywords) {
            const nameLower = file.name.toLowerCase();
            if (!rule.fileNameKeywords.some(keyword => nameLower.includes(keyword.toLowerCase()))) continue;
          }

          if (isTimeRelevant && settings.timeCheckEnabled && timeRangeForConfig) {
            const fileHour = new Date(file.lastModified).getHours();
            if (fileHour < timeRangeForConfig.start || fileHour > timeRangeForConfig.end) continue;
          }
